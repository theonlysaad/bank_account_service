...
public class BankAccountServiceApplication {
	...
	@Bean
	CommandLineRunner start(BankAccountRepositpry bankAccountRepositpry, CustomerRepository customerRepository){
		return args -> {
			Stream.of("khawla","saad","imane").forEach(c->{
			...
			customerRepository.findAll().forEach(customer -> {
				for (int i = 0; i <10 ; i++) {
					BankAccount bankAccount = BankAccount.builder()
							...
							.customer(customer)
							.build();
					bankAccountRepositpry.save(bankAccount);
				}
